...
void SmartPlayer::chooseMove1(const Board&b, Side original, Side player, int& besthole, int& value, int& deepness) const
{
    ...
    if(b.beansInPlay(Side::SOUTH)==0||b.beansInPlay(Side::NORTH)==0){
        ...
        if(temp.beans(player, 0) == temp.beans(player==Side::NORTH?Side::SOUTH:Side::NORTH, 0))
            ...
            value = -9999;
        return;
    }else if(deepness>3){ //60% of beans belongs to opponents side, should give up
        besthole = -1;
    ...
    for(int k=0; k<b.holes(); k++){
        if(b.beans(player, k+1)!=0){
            ...
            Board temp(b);
            Side temp_s;
            int temp_hole = -1, temp_value=0, temp_besthole=-2;
            ...
            if(player==temp_s && temp_hole ==0){
                int tempdeep = deepness;
                chooseMove1(temp, original, player, temp_besthole, temp_value, tempdeep);
            }else{
                Side temp_opp = player==Side::NORTH?Side::SOUTH:Side::NORTH; //opposite should be opposite of player
                //capture code
                if(player==temp_s&&temp_hole!=0&&temp.beans(temp_s, temp_hole)==1
                   &&temp.beans(temp_opp, temp_hole)!=0){
                    //do capture
                ...
                int tempdeep = deepness +1;
                chooseMove1(temp, original, temp_opp, temp_besthole, temp_value, tempdeep);
            }
            //if(endHole!=0&&m_board.beans(endSide, endHole)==1&&m_board.beans(opp, endHole)!=0){
            //need to check for capturing
            
            //check whether to update value or not
